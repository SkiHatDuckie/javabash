...
public class RobotCameraFile extends File {
    ...
    public RobotCameraFile(final Main game, String name) {
        ...
        terminal.addCommand("up", new Command() {
            public void execute(String[] args) {
                game.robot.move(Direction.UP);
            }
        });
        terminal.addCommand("right", new Command() {
            public void execute(String[] args) {
                game.robot.move(Direction.RIGHT);
            }
        });
        terminal.addCommand("down", new Command() {
            public void execute(String[] args) {
                game.robot.move(Direction.DOWN);
            }
        });
        terminal.addCommand("left", new Command() {
            public void execute(String[] args) {
                game.robot.move(Direction.LEFT);
            }
        });
    }
